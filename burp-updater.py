...
def check_for_updates(current_version: str) -> tuple:
    ...
    with console.status("[bold green]Fetching available versions...") as status:
        try:
            ...
            results = result_set['Results']
            for result in results:
                if (result['productType'] != 'pro' or result['productType'] != 'enterprise') and result['releaseChannels'][0] == 'Stable':
...
def uninstall_old_version() -> str:
    ...
    if not os.path.exists(dir_to_remove + '/uninstall'):
        raise Exception(f'Specified uninstaller({dir_to_remove + "/uninstall"}) does not exist!')
    if not prompt.lower() in ['y', 'yes']:
        return
    if os.geteuid() != 0:
        process = subprocess.run(['bash', '-c', f'sudo bash {dir_to_remove + "/uninstall"}'], capture_output=True, check=True)          
        if process.returncode != 0:
            raise Exception('Unable to remove old installation')
    else:
        process = subprocess.run(['bash', '-c', f'bash {dir_to_remove + "/uninstall"}'], capture_output=True, check=True)
        if process.returncode != 0:
            raise Exception('Unable to remove old installation')
...
def install_from_installer(installer_path: str):
    if os.geteuid() != 0:
        process = subprocess.run(['bash', '-c', f'sudo bash {installer_path}'], capture_output=True, check=True)          
        if process.returncode != 0:
            raise Exception('An error occured while installing!')
    else:
        process = subprocess.run(['bash', '-c', f'bash {installer_path}'], capture_output=True, check=True)
        if process.returncode != 0:
            raise Exception('An error occured while installing!')
